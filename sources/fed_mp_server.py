...
def run_fed_server(_args, _basic_port=BASIC_PORT):
    ...
    global_model = fed_model(data_shape[0], data_shape[1], True)
    ...
    for now_round in range(fed_round):
        ...
        threadings = []
        for c in range(len(_sample_client)):
        ...
        for sq in share_queue: assert sq.empty() == False
        rpcio_responses = [share_queue[c].get() for c in range(len(threadings))]
