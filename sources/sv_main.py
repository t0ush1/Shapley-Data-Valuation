...
from helper_alg import DEF_SHAP, TMC_SHAP, TMR_SHAP, GTB_SHAP, KG_SHAP, OR_SHAP, LOO_SHAP, DEF_SHAP_P, CC_SHAP,CC_Neyman_SHAP, CC_Berstin_SHAP, GTG_SHAP, K2G_SHAP
...
def exp_on_real_datasets(train_Setups = ['same'], datasets = ['emnist'], client_nums = [3, 6, 10, 15], used_models = ['cnn_model'], times = 1):
    ...
    # client_nums = [15]
    # used_models = ['cnn_model']   # +  ['linear_model']
    # sample_rounds = c
    ...
    algs  = dict({"Comb-Shapley":DEF_SHAP, "Perm-Shapley":DEF_SHAP_P, 'Extended-TMC':TMC_SHAP,
                       ...
                       'CC-Shapley':CC_SHAP, "Light Sampling": KG_SHAP, "GTG-Shapley":GTG_SHAP, "2-Greedy": K2G_SHAP
    })
    algs  = dict({"Comb-Shapley":DEF_SHAP, "Perm-Shapley":DEF_SHAP_P, 'Extended-TMC':TMC_SHAP,
                       "Extended-GTB":GTB_SHAP, "DIG-FL":LOO_SHAP,
                       'CC-Shapley':CC_SHAP, "Light Sampling": KG_SHAP, "2-Greedy": K2G_SHAP
    })
    ...
    ligh_shap= ['Light Sampling', "2-Greedy"]

    algs_res = {}

    # compare_algs= [DEF_SHAP, DEF_SHAP_P, TMC_SHAP, OR_SHAP, TMR_SHAP, GTB_SHAP, LOO_SHAP, CC_SHAP, KG_SHAP]
    # time_cost
                        ...
                        for alg_name, alg_func in algs.items():
                            ...
                            if alg_name in samp_shap:
                                if alg_name == 'Extended-TMC':
                                    ...
                                    # algs_res[alg_name] = (acc, acc_time)
                                    n = 50
                                    acc_tot, acc_time_tot = [0 for cid in range(cnum)], 0
                                    for i in range(n):
                                        acc, loss, acc_time, loss_time = alg_func(rec_sample_res, cnum, samp_rds//cnum, False)
                                        for cid in range(cnum):
                                            acc_tot[cid] += acc[cid]
                                        acc_time_tot += acc_time
                                    acc_avg, acc_time_avg = [a / n for a in acc_tot], acc_time_tot / n
                                    algs_res[alg_name] = (acc_avg, acc_time_avg)
                                    print("Extended-TMC shapley Value ({}):".format(acc_time_avg), '\n', acc_avg) 
                                ...
                                if alg_name == 'CC-Shapley':
                                    ...
                                    acc_tot, acc_time_tot = [0 for cid in range(cnum)], 0
                                    for i in range(n):
                                        acc, loss, acc_time, loss_time = alg_func(rec_sample_res, cnum, samp_rds, False)
                                        for cid in range(cnum):
                                            acc_tot[cid] += acc[cid]
                                        acc_time_tot += acc_time
                                    acc_avg, acc_time_avg = [a / n for a in acc_tot], acc_time_tot / n
                                    algs_res[alg_name] = (acc_avg, acc_time_avg)
                                    print("CC shapley Value ({}):".format(acc_time_avg), '\n', acc_avg) 
                            if alg_name in ligh_shap:
                                # samp_rds = int(max(math.sqrt((1.5)**cnum), 5))
                                acc, loss, acc_time, loss_time = alg_func(rec_sample_res, cnum, int(max(math.sqrt(2**cnum), 4)))
                                algs_res[alg_name] = (acc, acc_time)
...
if __name__ == "__main__":
    ...
    if TEST_MODE:
        ...
        exp_on_real_datasets(train_Setups = ['same'], datasets = ['adult'], client_nums = [3, 6, 10], used_models = ['xgboost_model'])
